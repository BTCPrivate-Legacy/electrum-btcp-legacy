...
;Variables
  ...
  !define PRODUCT_NAME "Electrum BTCP"
  !define PRODUCT_WEB_SITE "https://github.com/BTCPrivate/electrum-btcp"
  !define PRODUCT_PUBLISHER "Bitcoin Private"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-btcp-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoinprivate: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcoin-private" "" "URL:bitcoinprivate Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin-private" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin-private" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin-private\shell\open\command" "" "$\"$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\bitcoin-private"
